...
-- Config --
------------
Nino_Teleporter_TownHallText = "Mairie"
----------------
-- Fin Config --
----------------
...
function ENT:Draw()
    ...
    local len = surface.GetTextSize(Nino_Teleporter_TownHallText) + 120
    ...
    cam.Start3D2D(self:GetPos() + self:GetUp() * 80 + self:GetForward() * 3, self:GetAngles() + Angle(0,90,90), 0.20)
	   draw.SimpleTextOutlined(Nino_Teleporter_TownHallText, "TeleporterSystem80", 0, -150, Color(255, 255, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 10, Color(0, 0, 0))
	...
	ang:RotateAroundAxis(ang:Right(), 50)
		...
		cam.Start3D2D(pos +  Vector(0, 0, math.sin(CurTime())*5), a, 0.05)
			OdiumATM:MaterialRotated( 0, -800, 1250, 1150, pointtp, 0 )
		cam.End3D2D()
